...
dir.create(paste0("ssl_results/BacVir_", runname))
...
tar2 <-
  data.table(fin[[2]])[V1 == 1, res := "1"][V1 == 2, res := "2"][V1 == 3, res := "3"]
dtr <- data.table(fin[[1]], target = as.factor(tar2$res))
m2 = list(
  msr("classif.acc"),
  msr("classif.bacc"),
  msr("classif.ce"),
...
saveRDS(dtr,
  paste0("ssl_results/BacVir_", runname, "/L1_statesTR.rds"))
...
lnr$model
# Save result
saveRDS(lnr, file = paste0("ssl_results/BacVir_", runname, "/L1_learner.rds"))
...
dte <-
  readRDS(paste0(preloadGeneratorpath, "states/BacVir/", "test_", name, ".rds"))
...
sink(paste0("ssl_results/BacVir_", runname, "/L1_results.txt"))
print(results)
sink()
