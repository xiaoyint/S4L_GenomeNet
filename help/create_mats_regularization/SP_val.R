...
initializeGenerators(
  ...
  maxlen = 6700,
  step = 6700,
  fileLog = paste0(savepath, "fullgenval.csv")
)
fasval <- labelByFolderGeneratorWrapper(F, 4, NULL, 4, path.val, NULL, 6700)
...
rowsbefore <- 0
num_files <- 0
for (j in seq_along(path.val)) {
  num_files <- num_files + length(list.files(path.val[[j]]))
}
...
while (rowsnow < num_files) {
  ...
  files <- fread(paste0(savepath, "fullgenval.csv"), header = F)
  ar <- list()
  fidt <- list()
...
saveRDS(list(representations, label, file), paste0(savepath, "fullmatval.rds"))
