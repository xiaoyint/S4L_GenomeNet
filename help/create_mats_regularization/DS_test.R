...
cat(format(Sys.time(), "%d.%m.%y, %H:%M:%S :"), "LOAD TEST DATA\n")

gente <- gen_rds(paste0(path, "/test"), 1,  paste0(path, "/fullgentest.csv"))
...
rds_files <- list_fasta_files(corpus.dir = paste0(path, "/test"),
  format = "rds",
  file_filter = NULL)
num_samples <- 0
...
for (i in seq(num_samples)) {
  ...
  sequemce[[i]] <- as.array(dat[[1]] %>% tf$convert_to_tensor(),
      ...
      ncol = 4)
  label[[i]] <- dat[[2]]
  files <- fread(paste0(path, "/fullgentest.csv"), header = F)
  rowsnow <- nrow(files)
