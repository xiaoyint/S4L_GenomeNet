...
asd <- fastaLabelGenerator(
  ...
  maxlen = 6700,
  step = 6700,
  file_filter = filesval$file,
  fileLog = paste0(sub("\\/data\\/.*", "", path), "/MOT/fullgenval.csv"),
...
while (nrow(fread(paste0(
  sub("\\/data\\/.*", "", path), "/MOT/fullgenval.csv"
), header = F)) <= nrow(readRDS(paste0(
  sub("\\/data\\/.*", "", path), "/Motval.rds"
)))) {
  dat <- asd()
  representations[[i]] <-
  ...
  label[[i]] <- dat$Y[, 1]
  files <-
    fread(paste0(sub("\\/data\\/.*", "", path), "/MOT/fullgenval.csv"), header = F)
  rowsnow <- nrow(files)
...
saveRDS(list(representations, label, file),
  paste0(sub("\\/data\\/.*", "", path), "/MOT/fullmatval.rds"))
