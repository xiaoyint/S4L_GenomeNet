...
initializeGenerators(
  ...
  maxlen = 6700,
  step = 6700,
  fileLog = paste0(sub("\\/negative\\/.*", "", path.tes)[[1]], "/fullgentest.csv"),
)

fastest <-
  labelByFolderGeneratorWrapper(F, 1, NULL, 1, path.tes, NULL, 6700)
num_files <- 0
for (i in seq_along(path.tes)) {
  num_files <- num_files + length(list.files(path.tes[[i]]))
}
...
while (rowsnow < num_files) {
  ...
  files <- fread(paste0(sub("\\/negative\\/.*", "", path.tes)[[1]], "/fullgentest.csv"), header = F)
  ar <- list()
  fidt <- list()
...
saveRDS(list(representations, label, file), paste0(sub("\\/negative\\/.*", "", path.tes)[[1]], "/fullmattest.rds"))
