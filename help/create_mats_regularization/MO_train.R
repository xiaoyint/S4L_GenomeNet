...
asd <- fastaLabelGenerator(
  ...
  maxlen = 6700,
  step = 6700,
  file_filter = filestrain$file,
  fileLog = paste0(sub("\\/data\\/.*", "", path), "/MOT/fullgentrain.csv"),
...
while (nrow(fread(paste0(
  sub("\\/data\\/.*", "", path), "/MOT/fullgentrain.csv"
), header = F)) <= nrow(readRDS(paste0(
  sub("\\/data\\/.*", "", path), "/Mottrain.rds"
)))) {
  dat <- asd()
  representations[[i]] <-
  ...
  label[[i]] <- dat$Y[, 1]
  files <-
    fread(paste0(sub("\\/data\\/.*", "", path), "/MOT/fullgentrain.csv"), header = F)
  rowsnow <- nrow(files)
...
saveRDS(list(representations, label, file),
  paste0(sub("\\/data\\/.*", "", path), "/MOT/fullmattrain.rds"))
